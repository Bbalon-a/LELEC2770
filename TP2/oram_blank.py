...
class Client:
    ...
    def query(self, addr, write_data=None):
        ...
        """
        assert addr in range(self.capacity), "You're trying to read 
uninitialized memory !"
        ...
        #re-encrypt, except for data at addr, for which re-encrypt None 
and put
        #the data in res_data
        # </To be done by students>
        self._insert_block_at_root(addr, write_data if write_data is not 
None else res_data)
        return res_data

    def _insert_block_at_root(self, addr, data):
        self.pos[addr] = 
random.choice(BinaryTree.leafs_ids(self.tree_depth))
        new_block_idx = None
        for i in range(self.bucket_size):
            dec_block = 
self.crypto.dec(self.server.read(BinaryTree.root_id(), i))
    ...
    def _evict_bucket(self, node_id):
        ...
        # <To be done by students>
        # What should contain block_to_insert_left and 
block_to_insert_right? 
        # </To be done by students>
    ...
    def _child_insert(self, child_id, block_to_insert):
        for i in range(self.bucket_size):
            ...
            if dec_block is None and block_to_insert is not None:
                self.server.write(child_id, i, 
self.crypto.enc(block_to_insert))
                block_to_insert = None
            else:
                self.server.write(child_id, i, self.crypto.enc(dec_block))
        assert block_to_insert is None, "Congestion at node 
{}".format(child_id)


class BigStorageServer:
    """(Unstrusted) Backend storage server, with a simple API: it has a 
fixed
